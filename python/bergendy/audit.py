...
def render_audit_cli(summary: dict[str, Any]) -> str:
    lines = []
    lines.append("=" * 80)
    lines.append("         BOUNDARY: EXTERNAL-CHANGE DEPENDENCY AUDIT & RISK REGISTER")
    lines.append("=" * 80)
...
def render_audit_github_issue(summary: dict[str, Any]) -> str:
    lines = []
    lines.append("# Boundary: External Dependency Map & Risk Register\n")
    lines.append(f"Boundary mapped **{summary.get('total_callsites_mapped', 0)} external API touchpoints** across **{summary.get('total_providers_detected', 0)} providers** in this repository.\n")
    ...
    if at_risk:
        ...
        lines.append("| :--- | :--- | :--- | :--- | :--- | :--- |")
        for item in at_risk:
            lines.append(f"| **{item['provider_name']}** | `{item['package_name']}` | `{item['current_version']}` -> `{item['target_version']}` | **{item['callsites_count']} callsites** ({len(item['affected_files'])} files) | {item['breaking_change']} | `boundary fix --provider {item['provider_name'].lower()}` [MERGE_READY] |")
        lines.append("")
    ...
    lines.append("---")
    lines.append("> *Generated automatically by Boundary External-Change Intelligence.*")
    return "\n".join(lines)
...
def _changed_since(repo_root: str, state: dict[str, Any]) -> list[str]:
    ...
    for dirpath, dirnames, filenames in os.walk(repo_root, topdown=True):
        dirnames[:] = [d for d in dirnames if d not in {".git", "node_modules", ".next", "__pycache__", ".venv", "target", ".boundary"}]
        for fn in filenames:
...
def read_index_state(repo_root: str = ".") -> Dict[str, Any] | None:
    p = os.path.join(repo_root, ".boundary", STATE_FILE)
...
def write_index_state(repo_root: str, summary: dict[str, Any]) -> dict[str, Any]:
    ...
    os.makedirs(os.path.join(repo_root, ".boundary"), exist_ok=True)
    with open(os.path.join(repo_root, ".boundary", STATE_FILE), "w", encoding="utf-8") as f:
...
def run_audit(repo_root: str = ".", output_format: str = "cli", write_graph: bool = False) -> str:
    ...
    if write_graph:
        graph = build_dependency_graph(repo_root)
        os.makedirs(os.path.join(repo_root, ".boundary"), exist_ok=True)
        with open(os.path.join(repo_root, ".boundary", "graph.json"), "w") as f:
