...
from bergendy import __version__
import argparse
import dataclasses
import graphlib
import json
...
_logger = logging.getLogger("boundary.cli")

BOUNDARY_DIR = ".boundary"
TOPOLOGY_FILE = os.path.join(BOUNDARY_DIR, "topology.json")
CONFIG_FILE = os.path.join(BOUNDARY_DIR, "config.yaml")
...
def cmd_init(args):
    ...
    for sub in ("knowledge", "snapshots", "state", "logs", "boxes"):
        os.makedirs(os.path.join(workdir, BOUNDARY_DIR, sub), exist_ok=True)

    cfg_file = os.path.join(workdir, BOUNDARY_DIR, "config.yaml")
    ...
    # Detect Next.js and inject instrumentation.ts
    ...
    if os.path.exists(pkg_json_path):
        try:
            ...
            if "next" in deps:
                inst_files = [
                    os.path.join(workdir, "instrumentation.ts"),
                    os.path.join(workdir, "instrumentation.js"),
                    os.path.join(workdir, "src", "instrumentation.ts"),
                    os.path.join(workdir, "src", "instrumentation.js"),
                ]
                if not any(os.path.exists(p) for p in inst_files):
                    target = os.path.join(workdir, "src", "instrumentation.ts") if os.path.isdir(os.path.join(workdir, "src")) else os.path.join(workdir, "instrumentation.ts")
                    with open(target, "w", encoding="utf-8") as f:
                        f.write(
                            "// Auto-generated by Bergendy for Next.js App Router & Edge runtime\n"
                            "export async function register() {\n"
                            "  if (process.env.NEXT_RUNTIME === 'nodejs') {\n"
                            "    const { register } = await import('@bergendy/nextjs/instrumentation');\n"
                            "    await register();\n"
                            "  }\n"
                            "}\n"
                        )
                    print(f"[OK] Next.js detected: created {os.path.relpath(target, workdir)}")
                else:
                    print("[OK] Next.js detected: instrumentation hook configured")
        except Exception:
            pass
...
def cmd_doctor(args):
    ...
    graph_path = os.path.join(ws_root, ".boundary", "graph.json")
    indexed = os.path.isfile(graph_path)
    try:
        tcmd = _detect_test_command(ws_root)
    except Exception:
        tcmd = ""
    gh_connected = bool(os.environ.get("GITHUB_TOKEN") or os.environ.get("BOUNDARY_GITHUB_TOKEN")
                        or (os.environ.get("BOUNDARY_GITHUB_APP_ID") and os.environ.get("BOUNDARY_GITHUB_PRIVATE_KEY")))
    kb_state = "MISSING"
    if indexed:
        try:
    ...
    if daemon_running:
        ...
        mon_str = "CONFIGURED (Howl hunting — repos ready; run `boundary app serve` to listen for webhooks)"
    else:
        mon_str = "NOT ACTIVE (run `boundary app serve` to listen for webhooks)"

    print("================================================================================")
    print("                 BOUNDARY DOCTOR — product readiness                              ")
    print("================================================================================\n")
    ...
    if summary.get("configured"):
        ...
        print("AI provider:        NOT CONFIGURED — run `boundary auth` (needed for AI repair authoring)")
    print(f"Repository:         {ws_root}")
    print(f"Indexed:            {'YES' if indexed else 'NOT INDEXED — run `boundary index`'}")
    ...
    print("\nLocal Flow:  boundary check (free) → boundary auth → boundary consult (Howl) → boundary work (Hunt)")
...
def _snapshot_worktree(workspace_root: str, snapshot_id: str) -> str:
    """Copy the current worktree into ``.boundary/snapshots/<snapshot_id>/``.
    ...
    """
    snap_dir = os.path.join(workspace_root, ".boundary", "snapshots", snapshot_id)
...
def _launch_agent(
    ...
    missing).
    """
    cfg = load_config(os.path.join(workspace_root, ".boundary", "config.yaml"))
    ...
    if agent_name == "opencode":
        xdg_base = os.path.join(workspace_root, ".boundary", "xdg")
    ...
    extra_env["BOUNDARY_EXECUTION_ID"] = execution.execution_id
...
def cmd_exec_shim(args):
    """Internal: called by workspace shim scripts to launch an agent under Boundary.

    Usage: boundary _exec_shim <agent_name> <workspace_root> [argv...]
    """
...
def cmd_inspect(args):
    ...
    ws_root = find_workspace_root() or os.path.abspath(".")
    cfg_path = os.path.join(ws_root, ".boundary", "config.yaml")
    ...
    if getattr(args, "json", False):
        ...
        print(f"Boundary Workspace: {project_name}\n")
        print("COMPARTMENTS\n")
...
def cmd_active(args):
    """Show or set the currently active Boundary repository working context."""
    ...
    if repo:
        ...
        if current:
            ...
            print("No active repository set. Run `boundary connect` or `boundary active <repo>`.")
...
def cmd_connect_topology(args):
    ...
    if not source or not target:
        print("Usage: boundary connect-topology <source> <target>")
        return
    topology = _load_topology()
...
def cmd_connect(args):
    ...
    # Check if this was called via legacy 2-arg compartment connection: boundary connect <src> <target>
    source = getattr(args, "source", None)
    target = getattr(args, "target", None)
    ...
    if not client.token and not (client.app_id and client.private_key):
        print("================================================================================")
        print("                     BOUNDARY: CONNECT GITHUB ACCOUNT                             ")
        print("================================================================================\n")
        print("GitHub authentication required. Please set one of:")
        print("  export GITHUB_TOKEN=\"ghp_...\"")
        print("  or authenticate using GitHub CLI: `gh auth login`\n")
        print("Action required: Set GITHUB_TOKEN and run `boundary connect` again.")
        print("================================================================================")
        sys.exit(1)

    print("================================================================================")
    print("                      BOUNDARY: GITHUB REPOSITORY SELECTION                       ")
    print("================================================================================\n")
    ...
    if not selected_repos:
        print("No repository selected. Run `boundary connect --repo owner/repo`.")
        return
...
def cmd_run(args):
    ...
    ws_root = find_workspace_root() or os.path.abspath(".")
    cfg = load_config(os.path.join(ws_root, ".boundary", "config.yaml"))
    if cfg.workflows:
        print("Available Workflows:")
        for wname in sorted(cfg.workflows):
            print(f"  - {wname:<24} (run with: boundary run {wname})")
        return
    ...
    if os.path.exists(TOPOLOGY_FILE):
        ...
        if comps:
            ...
            print("Use `boundary workflow show` or `boundary check` for maintenance. `boundary run <workflow>` to execute.")
            print("No fallback execution performed.")
            return

    print("No workflow specified. Usage: boundary run <workflow_name>")


def cmd_exec(args):
    """Run a command or launch an agent inside an explicitly selected compartment."""
    if not args.cmd:
        print("Error: No command provided to exec. Usage: boundary exec [--compartment X] <command>")
        sys.exit(1)
    ...
    result = runner.run(
        ...
        env={"BOUNDARY_EXECUTION_ID": execution.execution_id},
    )
    if result.stdout:
        print(result.stdout)
    if result.stderr:
...
def cmd_wrap(args):
    ...
    if args.claude is not None:
        ...
        xdg_base = os.path.join(ws_root, ".boundary", "xdg")
        for sub in ["config", "data", "cache", "state"]:
            os.makedirs(os.path.join(xdg_base, sub), exist_ok=True)
        env_vars.update({
            "XDG_CONFIG_HOME": os.path.join(xdg_base, "config"),
    ...
    env_vars["BOUNDARY_EXECUTION_ID"] = execution.execution_id

    print(f"Boundary Agent Session #{session.session_id} active on Virtual Agent Lane '{lane_id}'.")
...
def _run_declared_workflow(ws_root: str, wf: WorkflowConfig, cfg: WorkspaceConfig) -> None:
    ...
    exec_mgr = ExecutionManager(workdir=ws_root)
    print(f"\nBOUNDARY WORKFLOW: {wf.name}  ({len(nodes)} node(s))\n")
    for n in nodes:
    ...
    for n in nodes:
        ...
        result = runner.run(
            ...
            env={"BOUNDARY_EXECUTION_ID": ex.execution_id},
        )
        if result.stdout:
...
def _run_workflow_file(ws_root: str, workflow_file: str, compartment_id: str, verbose: bool) -> None:
    ...
    print(f"Boundary Workflow Execution #{execution.execution_id}")
...
def cmd_workflow_branch(args):
    ...
    if not name:
        print("Error: Workflow branch name required. Usage: boundary workflow branch <name>")
        sys.exit(1)
    ...
    print(f"  To add steps:  boundary step {name} <file_or_command>")
    print(f"  To execute:    boundary workflow run {name}")
...
def cmd_hunt(args):
    """Hunt: autonomous repair starting from an identified Boundary finding.

    Usage: boundary hunt <id> [--create-pr] [--repo owner/repo]
    ...
    print("================================================================================")
    print("                         BOUNDARY HUNT: AUTONOMOUS REPAIR                         ")
    print("================================================================================\n")
...
def cmd_consult(args):
    ...
    if not has_valid_credentials():
        _print_auth_warning()
        print("Consult reasons with AI — static `boundary check` needs no key.")
        sys.exit(1)

    detections = [d for d in detect_changes(root_path) if d.outcome != NO_IMPACT]
    if not detections:
        print("Boundary Consult: no maintenance problems found. No changes made.")
        return
    ...
    title = f"[Boundary Consult] {len(items)} maintenance issue(s) in {repo}"
    resp = client.create_issue(repo=repo, title=title, body=body, labels=["boundary", "consult"])
...
def cmd_app(args):
    ...
    if action == "serve":
        ...
        secret = args.secret or os.environ.get("BOUNDARY_WEBHOOK_SECRET")
        if not secret and not getattr(args, "no_secret", False):
            ...
            print("Set BOUNDARY_WEBHOOK_SECRET, pass --secret, or use --no-secret for local debugging only.")
            sys.exit(2)
        ...
        print("================================================================================")
        print("               BOUNDARY GITHUB APP: CONTINUOUS WEBHOOK LISTENER                  ")
        print("================================================================================\n")
        ...
        try:
            ...
            print("\n[Boundary App] Stopped.")
    elif action == "status":
        print("[Boundary App] Status: Ready (Daemon not running locally).")


def cmd_mcp(args):
    """Run Boundary Model Context Protocol (MCP) server over stdio."""
    serve_stdio()
...
def cmd_maintain(args):
    ...
    if raw_dir == ".":
        ...
        if active_name:
            ...
            if rec and rec.get("workdir") and os.path.isdir(rec["workdir"]):
                root_dir = os.path.abspath(rec["workdir"])
    print("================================================================================")
    print("               BOUNDARY AUTONOMOUS MAINTENANCE LOOP: EXECUTION                   ")
    print("================================================================================\n")
    ...
    if not target_provider or target_provider == "auto":
        ...
        if detected_all:
            ...
            print("Run `boundary check` to audit, or pass --provider explicitly.")
            print("================================================================================")
            return
    ...
    if getattr(report, "error", None):
        ...
        print("Action required:       Run `boundary auth` to enable AI repair.")
...
def cmd_graph(args):
    ...
    print("================================================================================")
    print("                 BOUNDARY: EXTERNAL-CHANGE DEPENDENCY GRAPH                     ")
    print("================================================================================\n")
...
def cmd_providers(args):
    ...
    print("================================================================================")
    print("                    BOUNDARY PROVIDER CONTRACT REGISTRY                          ")
    print("================================================================================\n")
    for p in providers:
...
def cmd_pr(args):
    """Review a pull request using the Boundary PR Bot."""
    ...
    print("================================================================================")
    print("                      BOUNDARY AUTOMATED PR CONTRACT GUARD                       ")
    print("================================================================================\n")
...
def main():
    # Direct agent shortcut: `boundary claude` / `boundary opencode` / `boundary codex`
    ...
    parser.add_argument(
        "-v", "--version",
        action="version",
        version=f"bergendy {__version__}",
    )
    parser.add_argument(
        "-w", "--workflow",
        dest="workflow_flag",
        default=None,
        help="Create a new workflow branch (e.g. boundary -w invoice-pipeline)",
    )
    parser.add_argument(
        "--run",
        dest="run_flag",
        default=None,
        help="Run a workflow DAG directly (e.g. boundary --run invoice-pipeline)",
    )
    subparsers = parser.add_subparsers(dest="command", help=argparse.SUPPRESS)

    init_p = subparsers.add_parser("init", help="Initialize Boundary in the current repository")
    ...
    active_parser = subparsers.add_parser("active", help="Show or set the currently active Boundary repository working context")
    active_parser.add_argument("repo", nargs="?", default=None, help="Repository name to set as active (e.g. owner/repo)")

    disconnect_parser = subparsers.add_parser("disconnect", help="Disconnect active or specified repository from Boundary")
    ...
    apply_parser.add_argument("-c", "--commit", action="store_true", help="Automatically create a Git commit with Boundary trailers")
    ...
    undo_parser = subparsers.add_parser("undo", help="Reverse the last Boundary-managed apply operation")
    ...
    reviews_p = subparsers.add_parser("reviews", help="List past Boundary maintenance runs")
    ...
    app_p = subparsers.add_parser("app", help="Manage Boundary GitHub App and Webhook server")
    ...
    subparsers.add_parser("mcp", help="Run Boundary Model Context Protocol (MCP) server for AI assistants")
    ...
    index_p.add_argument("--write-graph", action="store_true", default=True, help="Persist .boundary/graph.json")
