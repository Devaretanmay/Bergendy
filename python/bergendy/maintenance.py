...
def record_migration_history(repo_dir: str, record: dict[str, Any]) -> None:
    ...
    history_dir = os.path.join(repo_dir, ".boundary")
...
def get_migration_history(repo_dir: str) -> list[dict[str, Any]]:
    """Retrieve verified migration history records from .boundary/history.json."""
    history_file = os.path.join(repo_dir, ".boundary", "history.json")
    if os.path.exists(history_file):
        try:
            with open(history_file, "r") as f:
                return json.load(f)
...
def run_maintenance_cycle(
    ...
    **_ignored: Any,
    ...
    snapshot_dir = os.path.join(repo_dir, ".boundary", "snapshot_tmp")
    ...
    if decision.strategy == "AI":
        ...
        if ai_planner is None:
            quarantine_error = (
                ...
                "Run `boundary auth` or set GROQ_API_KEY / ANTHROPIC_API_KEY / OPENAI_API_KEY."
            )
        else:
            ...
            if not target_files:
                for dirpath, dirnames, filenames in os.walk(repo_dir):
                    dirnames[:] = [d for d in dirnames if d not in {".git", "node_modules", ".next", "__pycache__", ".boundary"}]
                    for fn in filenames:
        ...
        quarantine_error = (
            ...
            "Run `boundary auth` to enable AI repair, or add a verified migration to the registry."
        )
    ...
    if create_pr and github_repo and files_modified > 0:
        branch_name = f"boundary/{p_spec.name}-v{actual_to.replace('.', '-')}"
        commit_msg = (
            f"migrate: {p_spec.display_name} {actual_from} -> {actual_to}\n\n"
            f"Detected and patched by Boundary autonomous maintenance engine.\n"
            f"Rules applied:\n" + "\n".join(f"- {d}" for d in all_rules)
        )
        pushed = git_commit_and_push(repo_dir, modified_paths, branch_name, commit_msg)

        if pushed:
            pr_title = f"boundary: migrate {p_spec.display_name} {actual_from} -> {actual_to}"
        ...
        if not pr_url and github_client:
            pr_resp = github_client.create_pull_request(
                ...
                labels=["boundary-maintenance", "verified-green"],
            )
