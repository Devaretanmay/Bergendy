...
_logger = logging.getLogger("boundary.consult")
...
class HowlBot:
    ...
    def consult(self, ctx: TriggerContext, require_ai: bool = False) -> dict[str, Any]:
        ...
        issue_url = None
        if ctx.repository:
            title = f"[Boundary Consult] {len(analysis.findings)} maintenance issue(s) detected in {ctx.repository}"
            try:
    ...
    def explain_pull_request(self, ctx: TriggerContext, require_ai: bool = False) -> dict[str, Any]:
        ...
        if not analysis.has_findings:
            body = (
                "## Boundary Consult: No Contract Impact Detected\n\n"
