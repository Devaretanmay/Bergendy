...
otherwise (public repos), or BOUNDARY_REPO_REMOTE_* overrides (tests,
mirrors, self-hosted).
"""
...
_logger = logging.getLogger("boundary.provisioning")


def cache_dir() -> str:
    base = os.environ.get("BOUNDARY_REPOS_DIR",
                           os.path.join(os.path.expanduser("~/.boundary"), "repos"))
...
def remote_for(repo_full_name: str, token: str | None = None) -> str:
    ...
    override = os.environ.get(f"BOUNDARY_REPO_REMOTE_{slug}")
    if override:
        return override
    if token:
