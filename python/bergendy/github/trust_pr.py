...
def generate_trust_pr_markdown(meta: TrustPRMetadata) -> str:
    ...
    summary_sentence = (
        ...
        f"Boundary found {found_count} affected callsite(s) -> patched {patched_count} -> "
        f"{test_summary} -> 0 unrelated files -> {status_tag}**"
    ) if found_count != patched_count else (
        f"**{meta.provider_name} {meta.from_version} -> {meta.to_version} {change_clause} -> "
        f"Boundary found {patched_count} affected callsite(s) -> patched those {patched_count} -> "
        f"{test_summary} -> 0 unrelated files -> {status_tag}**"
    )
    ...
    lines.extend([
        ...
        "Boundary verified zero unintended side effects across the entire codebase:",
