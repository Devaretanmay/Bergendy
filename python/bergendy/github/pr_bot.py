...
"""
Boundary GitHub App PR bot.

This is the first product milestone: install the app, open a PR, and Boundary
...
def run_on_pr_locally(
    ...
    Useful for:
    * local development
    * CI check jobs that want Boundary to post results
    """
...
def render_day0_onboarding_issue(repo: str, workdir: str | None = None) -> str:
    ...
    lines = [
        "-----------------------------------------",
        "   BOUNDARY DAY-0 REPOSITORY ONBOARDING   ",
        "-----------------------------------------",
        "",
        f"Boundary has mapped external API dependencies and contracts for `{repo}`.",
        "",
        ...
        "  - [OK] Auto-Fix Policy: Opt-in (Configurable via .boundary/config.yaml)",
...
def handle_installation_event(
    ...
    for repo_info in repos_data:
        ...
        try:
            client.create_issue(
                repo=repo_name,
                title="[BOUNDARY] Day-0 External Contract & API Dependency Map",
                body=issue_body,
                labels=["boundary", "maintenance"],
            )
...
def handle_issue_comment_event(
    ...
    """Handle issue_comment.* events mentioning @boundary.

    `@boundary` alone re-runs the pipeline on the PR; `@boundary explain`
    ...
    if not any(trigger in body_lower for trigger in ("@boundary", "@howl", "@hunt", "@consult", "@work")):
        return {"success": True, "event": event_type, "handled": False}
    ...
    # Route 1: Consult mode trigger (@howl, @consult, @boundary explain)
    if any(t in body_lower for t in ("@howl", "@consult", "@boundary explain")):
