...
"""
Boundary event model and shared maintenance pipeline.
    ...
    def run(self, ctx: TriggerContext) -> "PipelineResult":
        ...
        if not analysis.has_findings and not self.policy.always_report_clean:
            return PipelineResult(
                ...
                status="clean",
                comment_body="",
                status_description="Boundary: no external contract impact detected",
            )
    ...
    def _run_consult(self, ctx: TriggerContext, analysis: AnalysisResult) -> "PipelineResult":
        ...
        if not has_valid_credentials():
            return PipelineResult(
                ...
                analysis=analysis,
                status="refused",
                comment_body="Boundary Consult requires AI reasoning — no provider configured. "
                             "Run `boundary auth` to connect one.",
                status_description="Boundary Consult refused: no AI provider configured",
            )
        ...
        if ctx.pr_number is not None:
            ...
            try:
                title = f"[Boundary Consult] {len(items)} maintenance issue(s) detected in {ctx.repository}"
                self.client.create_issue(
                    repo=ctx.repository,
                    title=title,
                    body=body,
                    labels=["boundary", "consult"],
                )
            except Exception as e:
        ...
        return PipelineResult(
            ...
            status="consulted",
            comment_body=body,
            status_description=f"Boundary Consult: assessed {len(items)} maintenance issue(s), no code modified",
        )
...
def apply_fixes(
    ...
    """Apply fixes via Boundary Intelligence: AI-authored repair with contract context."""
    modified_files: list[str] = []
    unified_diffs: list[str] = []

    snapshot_dir = os.path.join(ctx.workdir, ".boundary", "snapshot_tmp")
    snapshotter = SnapshotManager(workdir=ctx.workdir, snapshot_dir=snapshot_dir)
    snapshotter.snapshot()
    setattr(ctx, "_snapshotter", snapshotter)

    intel = BoundaryIntelligence()
...
def surface_result(
    ...
    if analysis.verified and analysis.modified_files:
        ...
        if analysis.test_command and analysis.test_exit_code == 0:
            status_desc = f"Boundary: autonomous repair verified ({len(analysis.modified_files)} file(s) patched, tests green)"
        else:
            status_desc = f"Boundary: autonomous repair applied ({len(analysis.modified_files)} file(s) patched, test suite unconfigured)"
        ...
        status_desc = f"Boundary found {len(analysis.findings)} maintenance issue(s)"
    else:
        comment = render_verification_comment(analysis, ctx)
        status_desc = "Boundary: no external contract impact detected"
