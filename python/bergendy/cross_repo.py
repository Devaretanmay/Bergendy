...
def resolve_push_checkout(obs: Dict[str, Any], token: Any = None) -> Dict[str, Any]:
    ...
    may_fetch = bool(token) or bool(os.environ.get(f"BOUNDARY_REPO_REMOTE_{slug}"))
...
def _issue_title(source_repo: str, source_branch: str, target: Dict[str, Any]) -> str:
    return (f"[Boundary] `{target['repository']}/{target['branch']}` may be affected "
...
def request_hunt_repair(source_repo: str, source_branch: str, target_repo: str,
    ...
    snapshotter = SnapshotManager(
        workdir=checkout, snapshot_dir=os.path.join(checkout, ".boundary", "snapshot_tmp_hunt"))
    ...
    try:
        ...
        pr = client.create_pull_request(
            repo=target_repo,
            title=f"[Boundary] adapt {target_branch} to {source_repo}/{source_branch}",
