"""Execution trace - makes the Boundary lifecycle visible for development."""
...
def is_trace_enabled() -> bool:
    return os.environ.get("BOUNDARY_TRACE", "0") in ("1", "true", "yes", "on")
...
class Tracer:
    ...
    def header(self, request: str) -> None:
        ...
        lines = [
            ...
            f"  {_colorize(f'Boundary #{cid}', 'bold')}    {_colorize(ts, 'dim')}",
            f"  {_colorize(sep, 'cyan')}",
        ]
    ...
    def _format_event(self, event: str, elapsed: float, data: dict) -> str | None:
        ...
        if event == "box.created":
            path = data.get("path", "")
            return f"{prefix}  {_colorize('[ok]', 'green')} Boundary Created     {_colorize(path, 'dim')}"
        ...
        if event == "box.destroyed":
            return f"{prefix}  {_colorize('[ok]', 'green')} Boundary Released"
