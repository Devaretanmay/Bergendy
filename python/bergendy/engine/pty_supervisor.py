...
_logger = logging.getLogger("boundary.process_runner")
...
class PtySupervisor:
    ...
    def _resolve(self, binary_name: str) -> str:
        ...
        boundary_base = os.path.abspath(os.path.join(self.workdir, ".boundary"))
        path_dirs = os.environ.get("PATH", "").split(os.path.pathsep)
        for d in path_dirs:
            if not d:
                continue
            abs_d = os.path.abspath(d)
            if abs_d == boundary_base or abs_d.startswith(boundary_base + os.sep):
                continue
