...
or LLM-authored code inside a :class:`boundary.AgentBoundary` compartment.
...
:class:`boundary.sandbox.enforcer.SandboxEnforcer` still enforces the
...
from ..boundary import AgentBoundary, BoundaryConfig
...
_logger = logging.getLogger("boundary.hooks")
...
class ExecutionResult:
    """Structured outcome of running code inside a Boundary compartment.
...
class SandboxRunner:
    """Execute shell commands or code snippets inside an AgentBoundary.

    A fresh :class:`boundary.AgentBoundary` is created per :meth:`run`
    ...
    credential_rules
        ...
        :class:`boundary.sandbox.proxy.RouteConfig`).
    sandbox
    ...
    def _box(self) -> AgentBoundary:
        return AgentBoundary(config=BoundaryConfig(
    ...
    def run_code(
        ...
        *,
        ...
        The snippet is written under ``.boundary/tmp`` (outside the diff
        ...
        tmp_root = os.path.join(self.workdir, ".boundary", "tmp")
