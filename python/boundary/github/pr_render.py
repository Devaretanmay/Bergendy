...
def render_verification_comment(
    ...
    return f"Boundary checked {touchpoints} external API touchpoint(s). No contract violations detected. No changes made."
...
def render_maintenance_issue_comment(
    ...
    lines: list[str] = [
        "-----------------------------------------",
        "        BOUNDARY FOUND A MAINTENANCE ISSUE",
        "-----------------------------------------",
        "",
    ]
...
def _render_single_finding(
    ...
    lines.append("")
    lines.append("**Boundary can repair this automatically.**")
    lines.append("")
    return lines
...
def render_pr_summary(analysis: "AnalysisResult", ctx: "TriggerContext") -> str:
    ...
    if not analysis.has_findings:
        touchpoints = analysis.callsites_total or len(analysis.providers_detected) or 1
        return (f"## Boundary review: no contract impact\n\n"
    ...
    parts = [f"## Boundary review: {len(analysis.findings)} maintenance issue(s)", ""]
    for finding in analysis.findings:
...
def render_pr_footer(ctx: "TriggerContext") -> str:
    ...
    lines.append("Comment `@boundary` to re-run this review.")
    return "\n".join(lines)
...
def render_consult_issue(items: list[dict]) -> str:
    ...
    lines: list[str] = [
        "-----------------------------------------",
        "   BOUNDARY CONSULT: MAINTENANCE ADVISORY ",
        "-----------------------------------------",
        "",
    ]
    for item in items:
        ...
        if item.get("auto_repairable", True):
            lines.append("**Action:** Work mode can repair this automatically (`boundary work` or `@hunt repair`).")
        else:
