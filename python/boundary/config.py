"""Boundary workspace configuration loader.

Reads `.boundary/config.yaml` and returns typed config objects for compartments,
...
@dataclass
class BotConfig:
    """Boundary GitHub App behavior policy.
...
def load_config(config_path: str | None = None) -> WorkspaceConfig:
    """Load `.boundary/config.yaml`.  Returns safe defaults when not found."""
    if config_path is None:
        config_path = os.path.join(".boundary", "config.yaml")
    ...
    if not _YAML_AVAILABLE:
        warnings.warn(
            ...
            "to use .boundary/config.yaml. Falling back to default policy.",
            RuntimeWarning,
            stacklevel=2,
        )
        return _default_config()
    ...
    for dir_name in ("workflows", os.path.join(".boundary", "workflows")):
...
def is_boundary_workspace(path: str | None = None) -> bool:
    """Return True if *path* (or cwd) is inside a Boundary workspace."""
    check = os.path.abspath(path or ".")
    while True:
        if os.path.isdir(os.path.join(check, ".boundary")):
            return True
...
def find_workspace_root(path: str | None = None) -> str | None:
    """Walk up the directory tree looking for a .boundary/ directory."""
    check = os.path.abspath(path or ".")
    while True:
        if os.path.isdir(os.path.join(check, ".boundary")):
            return check
