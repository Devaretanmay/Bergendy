"""Boundary - kernel-level sandbox with an insulated runtime for isolated execution.

Two entry points:

* :class:`Boundary` : a normal outer compartment. A kernel sandbox plus a runtime that
...
* :class:`AgentBoundary` : an agent outer compartment (agent-oriented container). It
...
_logger = logging.getLogger("boundary")
...
class Boundary:
    ...
    def __init__(
        ...
        if config is not None:
            ...
            self.config = BoundaryConfig(workdir=workdir)
        self.workdir = os.path.abspath(self.config.workdir)
        self._snapshot_base = os.path.join(self.workdir, self.BOUNDARY_DIR, "snapshots")
    ...
    def add(self, compartment: Compartment) -> "Boundary":
    ...
    def edge(self, from_name: str, to_name: str) -> "Boundary":
    ...
    def run(
        ...
        BoundaryResult
        """
        task_desc = request or entry or "boundary.run()"
        self._started_at = time.time()
        _logger.info("Boundary %s running: %s", self.box_id, task_desc[:80])
    ...
    def _build_result(self, raw: dict[str, Any], elapsed: float) -> BoundaryResult:
        errors: list[str] = []
        completed: list[str] = []
        status = "success"
...
class AgentBoundary(Boundary):
    ...
    def __init__(
        ...
        if config is not None:
            ...
            config = BoundaryConfig(workdir=workdir, auto_modules=True)
