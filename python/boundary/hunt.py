...
"""Boundary Hunt: autonomous AI-first repair agent.
...
@dataclass
class HuntFinding:
    """A Boundary finding Hunt can start from. Starting point only."""
...
def persist_findings(repo_dir: str, findings: list[HuntFinding]) -> str:
    path = os.path.join(os.path.abspath(repo_dir), ".boundary", HUNT_DIRNAME, "findings.json")
...
def gather_context(repo_dir: str, finding: HuntFinding) -> HuntContext:
    ...
    try:
        hp = os.path.join(repo_dir, ".boundary", "history.json")
...
REASONING_SYSTEM = (
    "You are Hunt, Boundary's autonomous software maintenance repair agent. "
...
def create_sandbox(repo_dir: str, sha: str = "") -> SandboxResult:
    ...
    repo_dir = os.path.abspath(repo_dir)
    parent = os.path.join(repo_dir, ".boundary", HUNT_DIRNAME, "sandboxes")
...
def _audit_path(repo_dir: str, finding_id: str) -> str:
    path = os.path.join(os.path.abspath(repo_dir), ".boundary", HUNT_DIRNAME, finding_id, "audit.json")
    os.makedirs(os.path.dirname(path), exist_ok=True)
    return path
...
class LocalGitHubPublisher:
    ...
    def publish(self, repair: VerifiedRepair, ctx: HuntContext,
        ...
        if not repo_dir:
            return None
        branch = f"boundary/hunt-{repair.finding_id}"
        ...
        commit_msg = (
            ...
            f"Audit: .boundary/hunt/{repair.finding_id}/audit.json"
        )
        try:
        ...
        if not pushed:
            return None
        title = f"boundary(hunt): repair {repair.provider} {repair.version_from} -> {repair.version_to}"
        try:
...
def run_hunt(
    ...
    snapshotter = SnapshotManager(workdir=repo_dir,
                                  snapshot_dir=os.path.join(repo_dir, ".boundary",
...
class HuntLock:
    ...
    def __init__(self, repo_dir: str):
        self._path = os.path.join(os.path.abspath(repo_dir), ".boundary", "hunt.lock")
        self._fh: Any = None
        self.held = False
        self.degraded = False
...
def verify_promotion(
    ...
    unexpected = sorted(
        ...
        if c not in allowed and not c.startswith(".boundary/")
    )
    if unexpected:
...
def _promote_sandbox(sandbox_dir: str, repo_dir: str,
    ...
    for patch in patches:
        ...
        if not rel or rel.startswith(".boundary"):
            continue
