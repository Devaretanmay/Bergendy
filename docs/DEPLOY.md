...
|---|---|---|
| `BOUNDARY_WEBHOOK_SECRET` | **yes** | HMAC validation. The daemon refuses to serve without it (`--no-secret` is local-debug only). |
| `GITHUB_TOKEN` or App `BOUNDARY_GITHUB_APP_ID` + `BOUNDARY_GITHUB_PRIVATE_KEY` | yes for private repos / PR writes | Clone auth and PR comments. Public repos work anonymously for clones. |
...
| `BOUNDARY_REPOS_DIR`, `BOUNDARY_INSTALLATIONS_DIR` | no | Managed checkouts and install records. Persist both (volume `/data`). |
...
- **Local CLI:** `~/.boundary/credentials.json` (0600) plus per-installation
  scoped files. Operational convenience, explicitly not enterprise-grade.
- **Hosted daemon:** environment-injected secrets only (`BOUNDARY_WEBHOOK_SECRET`,
