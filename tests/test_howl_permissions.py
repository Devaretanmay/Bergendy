...
def test_howl_raises_permission_error_on_write_attempts():
    ...
    with pytest.raises(PermissionError, match="read-only authority"):
        howl.client.create_branch("octocat/repo", "main", "boundary/fix")
...
def test_hunt_client_can_invoke_write_endpoints():
    ...
    client._request = MagicMock(return_value={"ref": "refs/heads/boundary/fix", "number": 1})

    branch_res = client.create_branch("octocat/repo", "main", "boundary/fix")
    assert branch_res["ref"] == "refs/heads/boundary/fix"

    file_res = client.create_or_update_file("octocat/repo", "boundary/fix", "file.txt", "content", "msg")
    assert file_res["number"] == 1

    pr_res = client.create_pull_request("octocat/repo", "PR Title", "PR Body", "boundary/fix")
