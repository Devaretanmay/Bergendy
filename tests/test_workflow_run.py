"""Tests for `boundary workflow run <name>` (M5).
...
from boundary.cli.main import _topo_sort, cmd_workflow_run
from boundary.config import WorkflowNodeConfig
...
def _load_executions(ws) -> list[dict]:
    exec_dir = os.path.join(str(ws), ".boundary", "executions")
    records = []
...
def test_workflow_run_declared_dag(ws, fake_runner, capsys):
    ...
    assert "BOUNDARY WORKFLOW: feature-development" in out
...
def test_workflow_run_unknown_compartment(ws, capsys):
    config_path = ws / ".boundary" / "config.yaml"
...
def test_workflow_run_file_fallback(ws, fake_runner, capsys):
    ...
    sessions = list((ws / ".boundary" / "sessions").glob("*.json"))
