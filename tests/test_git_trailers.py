...
- `boundary diff --trailers` output
- `boundary commit` and `boundary apply --commit`
"""
...
from boundary.cli.main import _git_commit_execution, cmd_diff
from boundary.engine.execution import Execution, ExecutionKind, ExecutionManager
...
def test_git_commit_execution_staging_and_trailers():
    """_git_commit_execution stages files and creates a commit with Boundary trailers."""
    tmp = tempfile.mkdtemp()
    try:
        subprocess.run(["git", "init", "-b", "main"], cwd=tmp, check=True, capture_output=True)
        subprocess.run(["git", "config", "user.name", "Boundary Bot"], cwd=tmp, check=True)
        subprocess.run(["git", "config", "user.email", "bot@boundary.dev"], cwd=tmp, check=True)
...
def test_diff_with_trailers_in_json(monkeypatch):
    """`boundary diff --json` includes git_trailers in every execution dictionary."""
    tmp = tempfile.mkdtemp()
    try:
