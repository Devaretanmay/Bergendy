...
Covers `boundary diff` / `apply` / `undo` / `restore` and the fixed
...
from boundary.cli.main import cmd_apply, cmd_diff, cmd_restore, cmd_undo
from boundary.engine.execution import Execution, ExecutionKind, ExecutionManager, ExecutionStatus
...
def ws(tmp_path, monkeypatch):
    """A Boundary workspace; cwd is moved inside it."""
...
def _session_with_checkpoint(ws, snap_dir=None):
    ...
    snap_dir = snap_dir or os.path.join(str(ws), ".boundary", "snapshots", sess.session_id)
