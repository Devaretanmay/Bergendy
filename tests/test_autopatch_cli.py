...
def test_cli_diff_schema(capsys):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        sys.argv = ["boundary", "diff-schema", old_file, new_file]
        main()
...
def test_cli_diff_schema_json(capsys):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        sys.argv = ["boundary", "diff-schema", old_file, new_file, "--json"]
        main()
        captured = capsys.readouterr()
...
def test_cli_scan_api(capsys):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        sys.argv = ["boundary", "scan-api", "--root-dir", tmpdir, "--sdk", "stripe", "--method", "charges.create"]
        main()
        captured = capsys.readouterr()
...
def test_cli_workflow_order(capsys):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        sys.argv = ["boundary", "workflow-order", wf_file]
        main()
        captured = capsys.readouterr()
...
def test_cli_inventory(capsys):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        sys.argv = ["boundary", "inventory", "--root-dir", tmpdir]
        main()
        captured = capsys.readouterr()
