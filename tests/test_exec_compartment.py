"""Tests for `boundary exec --compartment X <agent>` (M4).
...
from boundary.cli.main import _launch_agent, _resolve_compartment, cmd_exec
from boundary.config import load_config
from boundary.hooks.base import ExecutionResult
...
def workspace(tmp_path):
    """A Boundary workspace with default/research/coding compartments."""
    (tmp_path / ".boundary").mkdir()
    (tmp_path / ".boundary" / "config.yaml").write_text(CONFIG_YAML, encoding="utf-8")
    return tmp_path
...
def _load_executions(workspace) -> list[dict]:
    exec_dir = os.path.join(str(workspace), ".boundary", "executions")
    records = []
...
def test_resolve_compartment_explicit_override_wins(workspace):
    cfg = load_config(os.path.join(str(workspace), ".boundary", "config.yaml"))
...
def fake_runner(monkeypatch):
    ...
    monkeypatch.setattr("boundary.cli.main.SandboxRunner", _FakeRunner)
    return _FakeRunner
...
def test_cmd_exec_process_sets_execution_marker_env(workspace, fake_runner, monkeypatch):
    ...
    assert env["BOUNDARY_EXECUTION_ID"] == exec_id
...
def test_launch_agent_records_execution_and_session(workspace, monkeypatch):
    ...
    _write_executable(
        str(realbin / "boundary_test_agent"),
        "#!/usr/bin/env bash\nexec /bin/echo \"$@\"\n",
    )
    ...
    returncode = _launch_agent(
        "boundary_test_agent", str(workspace), user_argv=["hello-exec"]
    )
    assert returncode == 0
    ...
    assert rec["command"] == ["boundary_test_agent", "hello-exec"]
    assert rec["compartment_id"] == "default"
    assert rec["returncode"] == 0
    assert rec["status"] == "COMPLETED"

    session_dir = workspace / ".boundary" / "sessions"
    sessions = list(session_dir.glob("*.json"))
    assert len(sessions) == 1
    session = json.loads(sessions[0].read_text(encoding="utf-8"))
    assert session["agent"] == "boundary_test_agent"
...
def test_launch_agent_respects_compartment_override(workspace, monkeypatch):
    ...
    _launch_agent("boundary_test_agent", str(workspace), user_argv=["a"], compartment_name="default")
    _launch_agent("boundary_test_agent", str(workspace), user_argv=["b"], compartment_name="research")
...
def test_launch_agent_missing_binary_exits_127(workspace, monkeypatch):
    monkeypatch.chdir(workspace)
    with pytest.raises(SystemExit) as exc:
        _launch_agent("boundary_no_such_agent_xyz", str(workspace))
...
def test_cli_exec_end_to_end(workspace):
    """`boundary exec --compartment research -- echo hi` records an Execution."""
    result = subprocess.run(
        [
            sys.executable, "-m", "boundary.cli.main",
