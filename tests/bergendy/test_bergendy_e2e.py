"""E2E tests for the compartment-centric Boundary API."""
...
from boundary.boundary import Boundary, AgentBoundary
...
class TestBoundaryInit(unittest.TestCase):
    ...
    def setUp(self):
        self.tmpdir = os.path.join(tempfile.gettempdir(), f"boundary_e2e_{uuid.uuid4().hex[:8]}")
    ...
    def test_plain_box_enables_no_insulation_by_default(self):
        """Boundary is just the box: no insulation auto-enabled."""
        box = AgentBoundary(workdir=self.tmpdir)
        self.assertTrue(box.config.auto_modules)
        box._box.insulate("Fix the bug")
        self.assertTrue(box._box._snapshot_enabled, "AgentBoundary should enable snapshots")
        self.assertIsNotNone(box._box.compressor, "AgentBoundary should enable compression")

        plain = Boundary(workdir=self.tmpdir)
        plain._box.insulate("Fix the bug")
        self.assertFalse(plain._box._snapshot_enabled, "plain Boundary insulates nothing")
        self.assertIsNone(plain._box.compressor, "plain Boundary ships empty")
...
class TestCompartmentFlow(unittest.TestCase):
    ...
    def setUp(self):
        self.tmpdir = os.path.join(tempfile.gettempdir(), f"boundary_flow_{uuid.uuid4().hex[:8]}")
        _make_repo(self.tmpdir)
