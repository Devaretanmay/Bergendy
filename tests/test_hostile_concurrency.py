...
from boundary.engine.execution import ExecutionManager, ExecutionKind, ExecutionStatus
from boundary.sandbox.snapshot import SnapshotManager
from boundary.sandbox.enforcer import SandboxEnforcer
...
def workspace_env(tmp_path):
    ...
    (ws / ".boundary" / "config.yaml").write_text(CONFIG_YAML, encoding="utf-8")
    return ws
...
def test_concurrent_snapshot_isolation(workspace_env):
    ...
    ex1.snapshot_dir = os.path.join(str(workspace_env), ".boundary", "snapshots", ex1.execution_id)
    ex2.snapshot_dir = os.path.join(str(workspace_env), ".boundary", "snapshots", ex2.execution_id)
...
def test_agent_crash_recovery(workspace_env):
    ...
    ex.snapshot_dir = os.path.join(str(workspace_env), ".boundary", "snapshots", ex.execution_id)
