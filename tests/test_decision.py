...
from boundary.intelligence import BoundaryIntelligence, Decision
...
def test_quarantine_for_known_sdk_rewrite_without_creds(tmp_path, monkeypatch):
    ...
    d = BoundaryIntelligence().decide("/tmp", "stripe", "11.18.0", "13.0.0", has_rewrites=True)
...
def test_quarantine_without_creds_or_rewrites(tmp_path, monkeypatch):
    ...
    for k in ("ANTHROPIC_API_KEY", "OPENAI_API_KEY", "GROQ_API_KEY", "BOUNDARY_LLM_KEY"):
        monkeypatch.delenv(k, raising=False)
    d = BoundaryIntelligence().decide(str(tmp_path), "twilio", "1.0", "2.0", has_rewrites=False)
    assert d.strategy == "QUARANTINE"
    assert d.confidence == 0.0
...
def test_no_direct_or_ai_cli_flag():
    res = subprocess.run(
        [sys.executable, "-m", "boundary.cli.main", "fix", "--help"],
...
def test_decide_for_source_future_kind_quarantines_without_creds(tmp_path, monkeypatch):
    ...
    for k in ("ANTHROPIC_API_KEY", "OPENAI_API_KEY", "GROQ_API_KEY", "BOUNDARY_LLM_KEY"):
        monkeypatch.delenv(k, raising=False)
    d = BoundaryIntelligence().decide_for_source(
