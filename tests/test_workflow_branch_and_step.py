...
Covers:
- `boundary branch <name>` / `boundary workflow branch <name>`
- `boundary step <workflow> <target>` with smart property auto-inference
...
def test_workflow_branch_creation():
    """`boundary branch <name>` creates workflows/<name>.yaml."""
...
def test_step_addition_and_autochaining():
    """`boundary step` appends steps and auto-chains depends_on."""
    tmp = tempfile.mkdtemp()
    old_cwd = os.getcwd()
    os.chdir(tmp)
    try:
...
def test_workflow_run_from_workflows_dir(monkeypatch):
    """`boundary workflow run <name>` executes workflow defined in workflows/<name>.yaml."""
    tmp = tempfile.mkdtemp()
    old_cwd = os.getcwd()
    os.chdir(tmp)
    try:
        os.makedirs(os.path.join(tmp, ".boundary"))
        with open(os.path.join(tmp, ".boundary", "config.yaml"), "w") as f:
...
def test_step_directory_batch_scan():
    """`boundary step <wf> src/` scans directory and adds all scripts sequentially."""
    tmp = tempfile.mkdtemp()
    old_cwd = os.getcwd()
    os.chdir(tmp)
    try:
...
def test_boundary_run_short_command(monkeypatch):
    """`boundary run <name>` executes declared workflow directly without 'workflow run'."""
    tmp = tempfile.mkdtemp()
    old_cwd = os.getcwd()
    os.chdir(tmp)
    try:
