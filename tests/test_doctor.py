...
from boundary.github.installations import REPO_READY, record_installation_event
...
def _env(tmp_path):
    ...
    env["BOUNDARY_INSTALLATIONS_DIR"] = str(tmp_path / "inst")
    for k in ("ANTHROPIC_API_KEY", "OPENAI_API_KEY", "GROQ_API_KEY", "BOUNDARY_LLM_KEY",
              "GITHUB_TOKEN", "BOUNDARY_GITHUB_TOKEN"):
        env.pop(k, None)
    return env
...
def test_doctor_ready_after_index(tmp_path):
    ...
    res = subprocess.run(
        [sys.executable, "-m", "boundary.cli.main", "doctor"],
...
def test_doctor_monitoring_names_howl(tmp_path, monkeypatch):
    env = _env(tmp_path)
    monkeypatch.setenv("BOUNDARY_INSTALLATIONS_DIR", str(tmp_path / "inst"))
...
def test_app_serve_requires_secret(tmp_path):
    env = _env(tmp_path)
    env.pop("BOUNDARY_WEBHOOK_SECRET", None)
    res = subprocess.run(
        [sys.executable, "-c",
         "from boundary.cli.main import cmd_app; "
