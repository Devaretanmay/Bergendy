"""Deep Edge-Case Test Suite for Boundary Workspace & Execution Architecture.

Covers:
1. Cross-agent merge conflict detection during `boundary apply`
...
4. Child process and subshell `BOUNDARY_EXECUTION_ID` boundary inheritance
...
from boundary.config import (
    ...
    find_workspace_root, is_boundary_workspace
)
...
def test_workflow_dag_node_failure_cascades_to_skipped(monkeypatch):
    ...
    try:
        ...
        monkeypatch.setattr("boundary.cli.main.SandboxRunner", _MockRunner)
...
def test_deeply_nested_workspace_root_discovery():
    """find_workspace_root correctly locates .boundary/ even 5 directories down."""
    os.chdir(REPO_ROOT)
    tmp = tempfile.mkdtemp()
    try:
        os.makedirs(os.path.join(tmp, ".boundary"))
        ...
        assert is_boundary_workspace(deep_dir) is True
...
def test_child_execution_id_inheritance():
    """Child processes receive BOUNDARY_EXECUTION_ID in extra_env."""
    os.chdir(REPO_ROOT)
    tmp = tempfile.mkdtemp()
    try:
        sup = PtySupervisor(workdir=tmp, extra_env={"BOUNDARY_EXECUTION_ID": "exec_parent_12345"})
        result = sup.capture(["sh", "-c", "echo $BOUNDARY_EXECUTION_ID"])
...
def test_execution_manager_path_traversal_protection():
    ...
    try:
        ...
        assert os.path.dirname(path_1) == os.path.join(tmp, ".boundary", "executions")
        assert os.path.basename(path_1) == "passwd.json"
        assert os.path.dirname(path_2) == os.path.join(tmp, ".boundary", "executions")
