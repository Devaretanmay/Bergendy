...
Covers:
- real-binary resolution that skips the workspace's own .boundary/bin shim dir
  (prevents shim self-recursion when the workspace is activated),
- the deterministic nested-workspace rule (innermost .boundary wins),
...
from boundary.cli.main import _resolve_real_binary
from boundary.config import find_workspace_root
...
def test_resolve_real_binary_skips_workspace_shim_dir(tmp_path, monkeypatch):
    """When activated, .boundary/bin shadows PATH; the shim must be skipped."""
    shim_dir = tmp_path / ".boundary" / "bin"
...
def test_resolve_real_binary_ignores_other_workspace_shims(tmp_path, monkeypatch):
    ...
    shim_dir = tmp_path / ".boundary" / "bin"
    other_shim_dir = tmp_path / "other-project" / ".boundary" / "bin"
    _write_executable(str(shim_dir / "claude"), "#!/usr/bin/env bash\n")
...
def test_find_workspace_root_innermost_wins(tmp_path):
    ...
    (inner / ".boundary").mkdir(parents=True)
    deep = inner / "src" / "deep"
