...
def test_watch_fires_once_then_quiet(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("BOUNDARY_REPO_REMOTE_ACME__BACKEND", _seed_repo_with_remote(tmp_path))
    for k in ("ANTHROPIC_API_KEY", "OPENAI_API_KEY", "BOUNDARY_LLM_KEY", "GITHUB_TOKEN"):
