...
def test_capture_with_extra_env():
    ...
    sup = PtySupervisor(workdir=".", extra_env={"BOUNDARY_TEST_VAR": "workspace_active"})
    result = sup.capture(["sh", "-c", "echo $BOUNDARY_TEST_VAR"])
    assert result.returncode == 0
...
def test_resolve_missing_binary():
    ...
    with pytest.raises(FileNotFoundError):
        sup.capture(["__boundary_nonexistent_binary__"])
