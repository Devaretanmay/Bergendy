...
pub fn render_inventory(inv: &Inventory) -> String {
    ...
    if critical > 0 {
        ...
        out.push_str("   Run `boundary autopatch` to generate verified migration PRs.\n");
    }
...
mod tests {
    ...
    fn inventory_scan_empty_dir() {
        let dir = std::env::temp_dir().join("boundary_inv_empty_test");
    ...
    fn inventory_scan_finds_stripe() {
        let dir = std::env::temp_dir().join("boundary_inv_stripe_test");
        let _ = std::fs::create_dir_all(&dir);
        std::fs::write(
            dir.join("billing.ts"),
    ...
    fn inventory_render_contains_providers() {
        let dir = std::env::temp_dir().join("boundary_inv_render_test");
        let _ = std::fs::create_dir_all(&dir);
        std::fs::write(
            dir.join("app.py"),
    ...
    fn inventory_scan_with_custom_providers() {
        let dir = std::env::temp_dir().join("boundary_inv_custom_test");
        let _ = std::fs::create_dir_all(&dir);
        std::fs::write(
            dir.join("client.ts"),
