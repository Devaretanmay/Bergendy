...
/// Structured semantic comparison between Boundary's patch and the human git diff.
...
pub struct ReplayEvidence {
    ...
    pub boundary_diff_blake3_hash: String,
...
pub fn classify_replay(evidence: &ReplayEvidence) -> (CausalReplayClassification, bool) {
    ...
    for p in &placeholder_prefixes {
        if evidence.lockfile_blake3_hash.starts_with(p)
            ...
            || evidence.boundary_diff_blake3_hash.starts_with(p)
        {
...
/// Compute structured semantic comparison between Boundary's patch and human diff.
pub fn compare_diffs_semantically(
    ...
    if human_files.contains(target_file) || boundary_diff.contains(target_file) {
        overlapping_files.push(target_file.to_string());
    }

    let boundary_added: Vec<&str> = boundary_diff
        .lines()
    ...
    let extra_edits = boundary_added.len().saturating_sub(overlapping_semantic_edits);
    let missed_edits = if overlapping_semantic_edits == 0 && !boundary_added.is_empty() { 1 } else { 0 };
    let unrelated_human_edits = human_added.len().saturating_sub(overlapping_semantic_edits);

    let semantic_score = if !boundary_added.is_empty() {
        if !human_added.is_empty() {
            overlapping_semantic_edits as f64 / boundary_added.len() as f64
        } else {
            1.0
        }
    } else {
...
mod tests {
    ...
    fn test_execute_and_record_command_produces_real_output_and_hash() {
        let temp = std::env::temp_dir().join("boundary_test_exec");
    ...
    fn test_classify_replay_rejects_synthetic_placeholders() {
        let evidence = ReplayEvidence {
            ...
            boundary_diff_blake3_hash: "def".into(),
