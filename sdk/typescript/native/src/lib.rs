...
pub fn version() -> String {
    boundary_core::version().to_string()
}

#[napi]
pub fn sandbox_supported() -> bool {
    boundary_core::sandbox::check_supported()
}

#[napi]
pub fn compress(content: String) -> String {
    boundary_core::compress(&content)
}
...
pub fn runtime_check_permission(policy_json: String, required_json: String) -> napi::Result<bool> {
    ...
    Ok(boundary_core::runtime::enforcer::check_permission_json(&policy, &required).is_ok())
}

#[napi]
pub fn runtime_check_command(cmd: String) -> napi::Result<bool> {
    Ok(boundary_core::runtime::enforcer::check_command(&cmd).is_ok())
}
...
pub fn runtime_snapshot(
    ...
    let mgr = boundary_core::runtime::snapshot::SnapshotManager::new(
        &workdir,
        &snapshot_dir,
...
pub fn runtime_restore(workdir: String, snapshot_dir: String) -> napi::Result<i32> {
    let mgr = boundary_core::runtime::snapshot::SnapshotManager::new(&workdir, &snapshot_dir, None);
...
pub fn runtime_credential_rewrite(routes_json: String, path: String) -> napi::Result<Option<String>> {
    let routes: Vec<boundary_core::runtime::credential::RouteConfig> = parse(&routes_json)?;
    for r in &routes {
        if r.matches(&path) {
...
pub fn runtime_credential_resolve(source: String) -> napi::Result<String> {
    let route = boundary_core::runtime::credential::RouteConfig {
...
fn build_runtime(configs: &str, edges: &[(String, String)]) -> napi::Result<boundary_core::runtime::compartments::Runtime> {
    #[derive(serde::Deserialize)]
    struct Spec {
        #[serde(default)]
        configs: Vec<boundary_core::runtime::compartments::CompartmentConfig>,
    }
    let spec: Spec = parse(configs)?;
    let mut rt = boundary_core::runtime::compartments::Runtime::new();
...
#[napi]
pub struct Runtime {
    inner: boundary_core::runtime::compartments::Runtime,
}
